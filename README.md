...
처음 읽는다면 **1 -> 2 -> 3 -> 5** 순서로 전체 그림을 먼저 보고, 세부 구현은 **8(AI)**, **10(실행)**, **11(트러블슈팅)** 에서 필요한 부분만 펼쳐보면 됩니다.
...
<details>
<summary><h3>1-2. 서비스 모듈 구성</h3></summary>

Spring Boot 기반 MSA 아키텍처로, 도메인별 책임을 분리하고 인프라 모듈(Gateway / Discovery)로 인증·서비스 탐색 같은 공통 관심사를 통합 관리합니다.

```
PeopleCore/
├── api-gateway/            # API 라우팅, JWT 검증
├── eureka-server/          # 서비스 디스커버리
├── common/                 # 공통 엔티티/설정/인터셉터 (별도 Maven 패키지로 배포)
...
PeopleCore는 Gateway를 단일 진입점으로 두고, 도메인 서비스는 Eureka 기반으로 탐색합니다. 서비스 간 후행 처리는 Kafka 이벤트로 분리하고, 검색은 Debezium CDC를 통해 Elasticsearch 색인으로 비동기 반영합니다.
...
Eureka Service Discovery
  ↓
hr-service / collaboration-service / search-service
  ↓
MySQL · Redis · Kafka · Elasticsearch · MinIO
```

> EKS 상세 배포 다이어그램은 추가 예정입니다.

### AI 아키텍처

```text
비민감 업무: User → Claude Haiku 4.5 → Tool Use → Search / Calendar / Approval
민감 업무:  User → Search RAG Context → EXAONE 3.5 Local → Answer
```

민감 데이터는 외부 LLM Tool 호출 경로에서 제외하고, 로컬 LLM이 검색 컨텍스트를 기반으로 답변만 생성하도록 분리했습니다.
...
| 영역 | 산출물 |
|------|--------|
| 화면 설계 | [Figma 화면 설계](https://www.figma.com/design/GRt4wS7G4Gc4oMM8hzOZSi/PeopleCore-%ED%99%94%EB%A9%B4-%EC%84%A4%EA%B3%84?node-id=15-71&t=PgQIPCy8W7eyuK2v-1) |
| API 문서 | [프로그램 사양서 및 단위테스트결과서](https://documenter.getpostman.com/view/51059727/2sBXqDrhar) |
| ERD | [ERDCloud](https://www.erdcloud.com/d/zu3piDR4rivmATs4d), 하단 이미지 자료 |
| 구현 화면 | 주요 기능 스크린샷 및 데모 GIF 추가 예정 |
...
로컬 LLM 후보(EXAONE 3.5, Qwen 2.5)를 한국어 업무 질의, 요약, 금액 추출, RAG 응답 기준으로 비교했습니다. 상세 결과는 [8. AI 정보](#8-ai-정보)의 LLM 모델 선정 섹션에 정리했습니다.
...
회사별 조직 요약, 용어집, Copilot 설정을 동적 시스템 프롬프트로 구성해 반복 컨텍스트 비용을 줄이는 방향으로 설계했습니다. 정량 비교 결과는 추가 예정입니다.
...
긴 문제 해결 기록은 도메인별로 접어두었습니다. 빠르게 훑을 때는 아래 요약을 먼저 보고, 필요한 항목만 펼쳐보면 됩니다.

| 도메인 | 대표 이슈 | 핵심 해결 |
|--------|-----------|-----------|
| 전자결재 | 승인·회수 충돌, 동시 채번, 상태 전이 복잡도 | JPA `@Version`, 비관적 락 + 자동 재시도, State/Strategy Pattern |
| 근태 관리 | 대용량 집계, 체크인 중복, 파티션 UPDATE 부하 | 월별 파티셔닝, UNIQUE 제약, 파티션 키 포함 native UPDATE |
| 휴가 관리 | 공휴일 조회 과다, 잔여 정합성 | 월 단위 벌크 조회 + Redis 캐시, 상태별 잔여 분리 + Ledger |
| 배치 / 스케줄러 | 다중 파드 중복 실행, 실행 이력 부재, misfire 정책 | Quartz JDBC 클러스터링, Spring Batch JobLauncher, 잡별 misfire 분기 |
