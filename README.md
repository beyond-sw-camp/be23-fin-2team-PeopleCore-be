# PeopleCore - 기업 운영 전반을 커스터마이징 가능하도록 설계한 통합 워크플로우 기반 엔터프라이즈 SaaS

## 소개

**Spring Boot MSA · 듀얼 LLM(Claude Haiku 4.5 + 로컬 EXAONE 3.5) · Elasticsearch BM25+kNN 하이브리드 검색** 으로 구현한 HR 기반 ERP SaaS입니다.

인사부터 급여·성과·전자결재·AI Copilot까지 단일 플랫폼으로 통합했으며, 민감/비민감 데이터 경로를 LLM 레벨에서 분리해 외부 유출을 차단했습니다.
...
├── collaboration-service/  # 전자결재, 전자결재, 캘린더, 알림
