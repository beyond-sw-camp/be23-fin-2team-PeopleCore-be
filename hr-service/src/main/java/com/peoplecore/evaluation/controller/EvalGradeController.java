...
public class EvalGradeController {
    ...
    // 6. 실제 vs 목표 분포 + 보정 건수
    //  - 응답: 등급별 target/actual/diff/status, mismatchCount, calibrationCount, isAllMatch
    //  - 페이지 상단 카드 5개 + "N개 등급 불일치" 배지 + "현재 보정 건수 N건" 한 번에
    ...
    // 8. 보정 이력
    //  - 응답: 보정된 사원 목록 (이름, autoGrade→adjustedGrade, 사유, 작성자, 일자)
