...
![HR / 기타 모듈 ERD](peoplecore-another.png)
...
![Collaboration 모듈 ERD](PEOPLECORE-PURPLE.png)
...
<details>
<summary><h3>1. 결재 문서 생명주기 관리</h3></summary>
...
- **임시저장** : 작성 중인 문서를 저장하고 이후 수정/삭제/제출 가능
- **결재 요청** : 결재선 지정 후 상신, 자동 채번 및 첫 번째 결재자에게 알림 발송
- **승인/반려** : 결재자별 의견 작성 가능, 반려 시 사유 필수 입력
- **회수** : 기안자가 진행 중인 문서를 취소
- **재기안** : 반려된 문서를 수정하여 다시 결재 요청

</details>

<details>
<summary><h3>2. 순차 결재 프로세스</h3></summary>

결재선의 `lineStep` 기반으로 순차 결재를 처리합니다.

- 이전 단계의 모든 결재자가 승인해야 다음 단계 결재자가 승인 가능
- 마지막 결재자 승인 시 문서 상태 자동으로 `APPROVED` 전환
- 결재 완료 시 서명이 포함된 HTML 문서를 생성하여 MinIO에 아카이빙

</details>

<details>
<summary><h3>3. 문서번호 자동채번</h3></summary>

회사별로 설정 가능한 문서번호 규칙을 기반으로 자동 채번합니다.

```
[슬롯1]-[슬롯2]-[슬롯3]-[날짜]-[순번]
예시: 인사-채용요청-260402-001
```

- **슬롯 타입** : 회사명, 부서코드, 부서명, 양식코드, 양식명, 커스텀 값 중 선택
- **순번 리셋 주기** : 연도별(YEAR), 월별(MONTH), 리셋 안함(NEVER)
- **순번 자릿수 / 구분자** 설정 가능

</details>

<details>
<summary><h3>4. 결재 위임</h3></summary>

결재자 부재 시 대결자에게 자동으로 결재 권한을 위임합니다.

- 위임 기간(시작일~종료일) 설정
- 위임된 결재 건은 `isDelegated=true`로 표시
- 관리자용 위임 관리 기능 별도 제공

</details>

<details>
<summary><h3>5. 문서함 시스템</h3></summary>

개인 문서함과 부서 문서함을 통해 문서를 체계적으로 관리합니다.

- **개인 문서함** : 사용자가 직접 폴더를 생성하고 문서를 분류
- **부서 문서함** : 대기함, 수신함, 발신함, 참조함, 열람함으로 구성
- **자동 분류** : 제목, 양식명, 기안자, 기안부서 조건을 AND로 결합하여 문서를 자동으로 개인 폴더에 분류 (우선순위 기반)

</details>

<details>
<summary><h3>6. 양식 관리</h3></summary>

결재 양식의 생성, 버전 관리, 권한 설정을 지원합니다.

- 폴더 계층 구조로 양식 분류
- 양식 버전 관리 (`formVersion`, `isCurrent`)
- 작성 권한 설정 (ALL / DEPT / PERSONAL)
- HTML 기반 양식 템플릿 (MinIO 저장)

</details>

<details>
<summary><h3>7. 공통 테이블 설계 - 엔티티 재사용을 통한 테이블 수 절감</h3></summary>
...
<details>
<summary><h3>1. 동시성 충돌 - 결재자 승인 vs 기안자 회수</h3></summary>
...
<details>
<summary><h3>2. 채번 동시성 - 동시 기안 시 중복 번호 발급</h3></summary>
...
<details>
<summary><h3>3. 상태 전이 복잡도 - if-else 분기 제거</h3></summary>
...
<details>
<summary><h3>4. 동적 검색 조건 - QueryDSL 활용</h3></summary>
...
<details>
<summary><h3>5. 문서번호 유연성 - 슬롯 패턴 (Strategy Pattern)</h3></summary>
...
<details>
<summary><h3>6. Kafka - 비동기 이벤트 기반 알림 및 캐시 무효화</h3></summary>
...
<details>
<summary><h3>7. Redis - 캐시 및 인증 토큰 관리</h3></summary>
